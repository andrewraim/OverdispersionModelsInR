fit.rcb.mle <- function(y, m, extra.tx = null.tx, var.names = NULL)
{
	...
	{
		theta <- theta.tx(phi)
		sum( d.rcb(Data$y, Pi = theta$Pi, rho = theta$rho,
			m = Data$m, log = TRUE) )
	}
	...
	fit.out$description <- "y[i] ~iid~ RCB(m[i], Pi, rho)"
	return(fit.out)
}

fit.rcb.x.mle <- function(y, m, X, extra.tx = null.tx, var.names = NULL)
{
	if (length(m) == 1) m <- rep(m, length(y))
	...
	loglik <- function(phi, Data)
	{
		...
		sum( d.rcb(Data$y, Pi = Pi, rho = theta$rho, m = Data$m, log = TRUE) )
	}
...
fit.rcb.xz.mle <- function(y, m, X, Z, extra.tx = null.tx, var.names = NULL)
{
	if (length(m) == 1) m <- rep(m, length(y))
